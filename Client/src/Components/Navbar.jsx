...
function Navbar() {
  ...
  const { backendUrl,user,setUser,setShowUserLogin,navigate,searchQuery,setSearchQuery,getCartCount } = useAppContext();
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
      ...
      <NavLink to="/" onClick={() => {setOpen(false);scrollTo(0,0)}}>
      ...
      <div className="hidden sm:flex items-center gap-8">
        ...
        <div onClick={() => navigate("/cart")} className="relative cursor-pointer">
          <img src={assets.nav_cart_icon} alt="cart" className='opacity-80'/>
          <button onClick={(e) => navigate('/cart')} className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full">{getCartCount()}</button>
        </div>
